...
class MyEditor implements ActionListener
{
 ...
 MyEditor()
 {
  ...
  jtf.setBounds(180,20,230,25);
   ...
   jta.setFont(new Font("varianda",Font.PLAIN,15));
   jta1.setFont(new Font("varianda",Font.PLAIN,15));
   ...
   if(e.getSource()==jbcompile)
   {
	 ...
	 if(!jtf.getText().equals(""))
	 {
	   try
	   {
		 ...
		 pw.println(s1);
		 pw.flush();
		 Process error=r.exec("C:\\Program Files\\Java\\jdk1.8.0_172\\bin\\javac.exe "+fname);
		 ...
		 while(true)
		 {
		   ...
		   if(result.equals(""))
		   {
		    jta1.setText("compilation sucessfuly"+fname);
			err.close();
		   }
		   else
		    jta1.setText(result);
       ...
       else if(e.getSource()==jbrun)
        {
          ...
          try
          {
			...
			Process p=r.exec("C:\\Program Files\\Java\\jdk1.8.0_172\\bin\\java.exe "+fn);
